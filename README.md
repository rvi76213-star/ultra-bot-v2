# 🤖 YOUR CRUSH BOT - ULTRA PROFESSIONAL

> Professional Messenger Bot with appstate.json support for Kiwi/Lemur Browser & GitHub Actions

## 🚀 Features
- ✅ appstate.json login system
- ✅ Kiwi/Lemur Browser compatible
- ✅ GitHub Actions auto-deploy
- ✅ Owner/Admin/User role system
- ✅ Fun commands (chor, murgi, abal, etc.)
- ✅ Photo management system
- ✅ Random delay (0.3-0.6s)
- ✅ Emergency stop feature
- ✅ No auto join
- ✅ Auto add friend/share
...
jobs:
  deploy:
      ...
      run: |
        if [ -f "src/secure/appstate.json" ]; then
          ...
          if [ $FILE_SIZE -gt 100 ]; then
            echo "✅ appstate.json is valid"
          else
            echo "❌ appstate.json is empty or invalid"
    ...
    - name: Start Messenger Bot
      ...
      run: |
        echo "🚀 Starting YOUR CRUSH Bot..."
        echo "👑 Developer: RANA (MASTER 🪓)"
        echo "📍 Location: Faridpur, Dhaka, Bangladesh"
        echo ""
    ...
    - name: Keep bot running
      if: steps.check_appstate.outputs.appstate_exists == 'true'
      run: |
        echo "⏰ Bot will run for 5 minutes..."
        ...
        for i in {1..300}; do
          ...
          if [ -z "$BOT_PID" ] || ! ps -p $BOT_PID > /dev/null; then
            echo "❌ Bot stopped unexpectedly at $i seconds"
            break
          fi
          ...
          if [ $((i % 30)) -eq 0 ]; then
            MINUTES=$((i / 60))
            SECONDS=$((i % 60))
            echo "✅ Bot running for ${MINUTES}m ${SECONDS}s"
          fi
          
          sleep 1
        done
        
        echo "⏰ 5 minutes completed"
